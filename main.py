...
def get_curr_data(page):
    return {
       'email_start': str(page*20+1),
       'coll_start': str(page*20+1),
       'citman_start': str(page*20+1),
       'EntrezSystem2.PEntrez.PubMed.Pubmed_ResultsPanel.Entrez_Pager.cPage': str(page+1),
       'EntrezSystem2.PEntrez.PubMed.Pubmed_ResultsPanel.Entrez_Pager.cPage': str(page+1),
    }

def get_xml():
    curr_data = {**data, **get_curr_data(1)}
...
def extract_author(author):
    ...
    for affiliation in affiliations:
        ...
        if len(emails) > 0:
            email = emails[0]
    return dict(
        lastName=str(author.lastname.string),
        foreName=str(author.forename.string),
